...
class Splatfacto2DGSModelConfig(ModelConfig):
    """Splatfacto2DGS Model Config, nerfstudio's implementation of Gaussian Splatting"""
...
class Splatfacto2DGSModel(Model):
    ...
    Args:
        config: Splatfacto2DGS configuration to instantiate model
    """
    ...
    def load_state_dict(self, dict, **kwargs):  # type: ignore
        ...
        lpips_keys = [k for k in dict.keys() if "lpips" in k]
        for k in lpips_keys:
            dict.pop(k)
        ...
        with torch.no_grad():
            ...
            if do_densification:
                ...
                if hasattr(self, "normals"):
                    self.normals = torch.cat([self.normals, split_params["normals"], dup_params["normals"]], dim=0)
                if hasattr(self, "src_normals"):
                    self.src_normals = torch.cat([self.src_normals, self.src_normals[splits.repeat(nsamps)], self.src_normals[dups]], dim=0)
    ...
    def cull_gaussians(self, extra_cull_mask: Optional[torch.Tensor] = None):
        ...
        if hasattr(self, "normals"):
            self.normals = self.normals[~culls]
        if hasattr(self, "src_normals"):
            self.src_normals = self.src_normals[~culls]
    ...
    def split_gaussians(self, split_mask, samps):
        ...
        if hasattr(self, "normals"):
            normals = self.normals[split_mask].repeat(samps, 1)
            out["normals"] = normals
        if hasattr(self, "src_normals"):
            src_normals = self.src_normals[split_mask].repeat(samps, 1)
            out["src_normals"] = src_normals
        for name, param in self.gauss_params.items():
            if name not in out:
    ...
    def dup_gaussians(self, dup_mask):
        ...
        if hasattr(self, "normals"):
            new_dups["normals"] = self.normals[dup_mask]
        if hasattr(self, "src_normals"):
            new_dups["src_normals"] = self.src_normals[dup_mask]
        return new_dups
    ...
    def get_outputs(self, camera: Cameras) -> Dict[str, Union[torch.Tensor, List]]:
        ...
        if self.config.output_depth_during_training or not self.training or self.config.depth_loss:
            render_mode = "RGB+ED"
        else:
            render_mode = "RGB"
        ...
        if self.training and info[self.config.key_for_gradient].requires_grad:
            info[self.config.key_for_gradient].retain_grad()
        self.xys = info[self.config.key_for_gradient]  # [1, N, 2]
        self.radii = info["radii"][0]  # [N]
        self.normals = info["normals"][0].detach()
        alpha = alpha[:, ...]
        ...
        normals_for_viewer = F.normalize(normals, p=2, dim=-1).squeeze(0)
        normals_for_viewer = (normals_for_viewer + 1) / 2
        ...
        return {
            ...
            "normals_for_viewer": normals_for_viewer,
    ...
    def get_loss_dict(self, outputs, batch, metrics_dict=None) -> Dict[str, torch.Tensor]:
        ...
        render_distort = outputs["render_distort"]
        ...
        if "weight_mask" in batch:
            weight_mask = self._downscale_if_required(batch["weight_mask"])
            weight_mask = weight_mask.to(self.device)
            assert weight_mask.shape[:2] == gt_img.shape[:2] == pred_img.shape[:2]
        else:
            weight_mask = torch.ones_like(gt_img)

        Ll1 = (weight_mask * torch.abs(gt_img - pred_img)).mean()
        simloss = 1 - self.ssim(gt_img.permute(2, 0, 1)[None, ...], pred_img.permute(2, 0, 1)[None, ...])
        
        if hasattr(self.config, "use_lpips") and self.config.use_lpips:
            self.lpips(gt_img.permute(2, 0, 1)[None, ...], pred_img.permute(2, 0, 1)[None, ...])
            lpips_loss = (self.lpips.loss[..., None] * weight_mask).mean()
        else:
            lpips_loss = torch.tensor(0.0).to(self.device)
        ...
        if self.config.dist_loss:
            ...
            distloss = distloss * curr_dist_lambda
        else:
            distloss = torch.tensor(0.0).to(self.device)

        l1_coef = 1 - self.config.ssim_lambda if not hasattr(self.config, "l1_loss_mult") else self.config.l1_loss_mult
        lpips_coef = 0 if (not hasattr(self.config, "use_lpips") or not self.config.use_lpips) else self.config.lpips_loss_mult

        loss_dict = {
            "main_loss": l1_coef * Ll1 + self.config.ssim_lambda * simloss + lpips_loss * lpips_coef,
            "scale_reg": scale_reg,
